...
class engine3D {
    ...
    private:
        ...
        void DrawTriangle(Graphics2D* g2d, int x1, int y1, int x2, int y2, int x3, int y3) {
            g2d->drawLine(x1, y1, x2, y2, 255);
            g2d->drawLine(x2, y2, x3, y3, 255);
            g2d->drawLine(x3, y3, x1, y1, 255);
        };
    ...
    public:
        ...
        bool onUpdate(OswHal* hal, float fElapsedTime) {
            ...
            for (auto tri : meshCube.tris) {
                ...
                triProjected.p[0].x *= 0.5f * 320.0f; //width
                triProjected.p[0].y *= 0.5f * 320.0f; //height
                triProjected.p[1].x *= 0.5f * 320.0f;
                triProjected.p[1].y *= 0.5f * 320.0f;
                triProjected.p[2].x *= 0.5f * 320.0f;
                triProjected.p[2].y *= 0.5f * 320.0f;
